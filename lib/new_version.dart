  ...
  Future<VersionStatus?> _getAndroidStoreVersion(PackageInfo packageInfo) async {
    ...
    if (additionalInfoElements.isNotEmpty) {
      ...
      final infoScriptElement = scriptElements.firstWhere(
        /// NACHIKET Changed this for new Google Play Store API (ds:4 to ds:5)
        (elm) => elm.text.contains('key: \'ds:5\''),
        //
      );

      final param = infoScriptElement.text.substring(20, infoScriptElement.text.length - 2).replaceAll('key:', '"key":').replaceAll('hash:', '"hash":').replaceAll('data:', '"data":').replaceAll('sideChannel:', '"sideChannel":').replaceAll('\'', '"');
